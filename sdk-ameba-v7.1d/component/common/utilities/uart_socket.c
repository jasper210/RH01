...
*************************************************************************/
#if 1
static unsigned char gv_uart_first_not_00_flag = 0;
static void uart_irq(uint32_t id, SerialIrq event)
{
	...
	if(event == RxIrq) {
		...
		vl_tmp = serial_getc(&u->sobj);
		//printf("%d", vl_tmp);
		if (0 == gv_uart_first_not_00_flag)
		{
			if (vl_tmp == 0)
			{
				return;
			...
			{
				gv_uart_first_not_00_flag = 1;
			}
		}
		
		u->recv_buf[u->prxwrite++] = vl_tmp;
...
uart_socket_t* uart_open(uart_set_str *puartpara)
{
	...
	serial_baud(&u->sobj,9600);
	serial_format(&u->sobj, 8, ParityNone, 1);
	
	serial_irq_handler(&u->sobj, uart_irq, (uint32_t)u);
...
#define WAPI_DUMP_HEX1(PTR, LEN, STR)   \
do {\
	int i; \
	printf("%s", (STR)); \
    for(i = 0; i < (LEN); i++)\
...
{
	printf("entry uart socket\n");
	//char tx_data[] = {0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06};
    char tx_data[] = {0xF4, 0xF5, 0x00, 0x40, 0x0B, 0x00, 0x00, 0x00, 0x00, 0xFE, 0x01, 0x00, 0x00, 0x0A, 0x04, 0x00, 0x01, 0x58, 0xF4, 0xFB};
	...
	while(1)
	{
		...
		if(count++ == 50){
			...
			WAPI_DUMP_HEX1(tx_data,sizeof(tx_data),"TX");
			count = 0;
		}
			...
			if(FD_ISSET(uart_fd, &readfds))
			{
				...
				if(read_len > 0)
				{
					//uart_print_data("RX:", rxbuf, read_len);
                     WAPI_DUMP_HEX1(rxbuf,read_len,"RX");
