...
/** @defgroup  PERIPH_APP Peripheral Application
    * @brief This file handles application routines.
    * @{
    */
/*============================================================================*
 *                              Variables
...
T_APP_RESULT app_profile_callback(T_SERVER_ID service_id, void *p_data)
{
    ...
    if (service_id == SERVICE_PROFILE_GENERAL_ID)
    {
		...
		printf(" #WWW [%s]-%d,%d,%d\n", __FUNCTION__, __LINE__, p_param->eventId, p_param->event_data.send_data_result.attrib_idx);
    ...
    else  if (service_id == simp_srv_id)
    {
		...
		//printf(" #WWW [%s]-%d, %d,%d\n", __FUNCTION__, __LINE__, p_simp_cb_data->msg_type, p_simp_cb_data->msg_data.read_value_index);
        ...
        case SERVICE_CALLBACK_TYPE_READ_CHAR_VALUE:
            {
                ...
                {
                    uint8_t al_read_buf[255] = {0};
					uint32_t read_buf_len = 0;
					hsn_ble_config_tlv_read_reply(al_read_buf, &read_buf_len, p_simp_cb_data->msg_data.read_value_index);
					if(read_buf_len > 0) {
						WAPI_DUMP_HEX(al_read_buf,read_buf_len,"Read data: ");
						simp_ble_service_set_parameter(SIMPLE_BLE_SERVICE_PARAM_V1_READ_CHAR_VAL, read_buf_len, al_read_buf);
					}
                }
            }
            break;
        case SERVICE_CALLBACK_TYPE_WRITE_CHAR_VALUE:
            {
                ...
                {
				case SIMP_WRITE_V2_2:
					{
						WAPI_DUMP_HEX(p_simp_cb_data->msg_data.write.p_value,p_simp_cb_data->msg_data.write.len,"Write data:");
						hsn_ble_config_tlv_write_parse(p_simp_cb_data->msg_data.write.p_value, p_simp_cb_data->msg_data.write.len);
						//simp_ble_service_set_parameter(SIMPLE_BLE_SERVICE_PARAM_V1_READ_CHAR_VAL, p_simp_cb_data->msg_data.write.len, p_simp_cb_data->msg_data.write.p_value);
					}
					break;
                default:
                    break;
