...
#define SIMPLE_BLE_SERVICE_CHAR_V1_READ_INDEX           0x04
#define SIMPLE_BLE_SERVICE_CHAR_V1_READ_5_INDEX         0x05

#define SIMPLE_BLE_SERVICE_CHAR_V2_WRITE_INDEX          0x05
#define SIMPLE_BLE_SERVICE_CHAR_V2_WRITE_4_INDEX        0x04
#define SIMPLE_BLE_SERVICE_CHAR_V2_WRITE_3_INDEX        0x03
                                            ...
                                            uint16_t attrib_index, uint16_t offset, uint16_t *p_length, uint8_t **pp_value)
{
	...
	//printf(" #WWW [%s]-%d, attrib_index: %d, %d, %d\n", __FUNCTION__, __LINE__, attrib_index, offset, conn_id);
    switch (attrib_index)
    {
    default:
    ...
    case SIMPLE_BLE_SERVICE_CHAR_V1_READ_INDEX:
		...
		break;
	case SIMPLE_BLE_SERVICE_CHAR_V1_READ_5_INDEX:
		{
                                ...
                                uint16_t length, uint8_t *p_value)
{
	...
	//printf(" #WWW [%s]-%d, attrib_index: %d,%d\n", __FUNCTION__, __LINE__, attrib_index, length);
}
/**
                                            ...
                                            P_FUN_WRITE_IND_POST_PROC *p_write_ind_post_proc)
{
	...
	//printf(" #WWW [%s]-%d, attrib_index: %d\n", __FUNCTION__, __LINE__, attrib_index);
    *p_write_ind_post_proc = simple_write_post_callback;
	...
	else if (SIMPLE_BLE_SERVICE_CHAR_V2_WRITE_4_INDEX == attrib_index)
	{
		/* Make sure written value size is valid. */
        if (p_value == NULL)
        {
        ...
        {			
            ...
            callback_data.msg_type = SERVICE_CALLBACK_TYPE_WRITE_CHAR_VALUE;
            callback_data.conn_id  = conn_id;
            callback_data.msg_data.write.opcode = SIMP_WRITE_V2_2;
            callback_data.msg_data.write.write_type = write_type;
            callback_data.msg_data.write.len = length;
            callback_data.msg_data.write.p_value = p_value;
	...
	else if (SIMPLE_BLE_SERVICE_CHAR_V2_WRITE_3_INDEX == attrib_index)
	{
		if (p_value == NULL)
        {
            cause  = APP_RESULT_INVALID_VALUE_SIZE;
        ...
        else
        {
            ...
            callback_data.msg_type = SERVICE_CALLBACK_TYPE_WRITE_CHAR_VALUE;
            callback_data.conn_id  = conn_id;
            callback_data.msg_data.write.opcode = SIMP_WRITE_V2_3;
            callback_data.msg_data.write.write_type = write_type;
            callback_data.msg_data.write.len = length;
            callback_data.msg_data.write.p_value = p_value;
    ...
    {
    	printf(" #WWW [%s]-%d, attrib_index 0x%x, length %d\n", __FUNCTION__, __LINE__, attrib_index, length);
                                     ...
                                     uint16_t length)
{
	...
	printf(" #WWW [%s]-%d, attrib_index 0x%x, length %d\n", __FUNCTION__, __LINE__, conn_id, service_id);
