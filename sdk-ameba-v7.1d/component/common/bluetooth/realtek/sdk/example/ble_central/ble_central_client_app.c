...
void ble_central_app_handle_dev_state_evt(T_GAP_DEV_STATE new_state, uint16_t cause)
{
    ...
    if (ble_central_gap_dev_state.gap_init_state != new_state.gap_init_state)
    {
        if (new_state.gap_init_state == GAP_INIT_STATE_STACK_READY)
        {
            ...
            BLE_PRINT("[%s]le_gen_rand_addr result = %x\n\r",__func__,result);
            result = le_set_rand_addr(bt_addr);
            BLE_PRINT("[%s]le_set_rand_addr result = %x\n\r",__func__,result);
...
T_APP_RESULT ble_central_app_gap_callback(uint8_t cb_type, void *p_cb_data)
{
    ...
    case GAP_MSG_LE_SCAN_INFO:
		...
		extern void hsn_bt_scan_result(char *adv_type, char* remote_addr_type, unsigned char* bd_addr, signed char bt_rssi);

		//if (GAP_ADV_EVT_TYPE_UNDIRECTED == p_data->p_le_scan_info->adv_type)
		{
			hsn_bt_scan_result(adv_type, remote_addr_type, p_data->p_le_scan_info->bd_addr, p_data->p_le_scan_info->rssi);
		}
