...
void bt_harmony_adapter_app_handle_conn_state_evt(uint8_t conn_id, T_GAP_CONN_STATE new_state, uint16_t disc_cause)
{
	...
	case GAP_CONN_STATE_DISCONNECTED: {
		...
		bt_harmony_adapter_app_send_callback_msg(BHA_CALLBACK_MSG_DISCONNECTED, conn_data);
	...
	case GAP_CONN_STATE_CONNECTED: {
		...
		conn_data->serverID = 0;
		memcpy(conn_data->bt_addr.addr, RT_remote_bd, OHOS_BD_ADDR_LEN);
		bt_harmony_adapter_app_send_callback_msg(BHA_CALLBACK_MSG_CONNECTED, conn_data);
	}
	break;
...
void bt_harmony_adapter_app_handle_conn_mtu_info_evt(uint8_t conn_id, uint16_t mtu_size)
{
	...
	bt_harmony_adapter_app_send_callback_msg(BHA_CALLBACK_MSG_MTU_CHANGED, mtu_data);
}
...
T_APP_RESULT bt_harmony_adapter_app_profile_callback(T_SERVER_ID service_id, void *p_data)
{
	...
	if (service_id == SERVICE_PROFILE_GENERAL_ID) {
		...
		case PROFILE_EVT_SRV_REG_AFTER_INIT_COMPLETE: {
			...
			bt_harmony_adapter_app_send_callback_msg(BHA_CALLBACK_MSG_SERVICE_ADDED, callback_data);
		}
		break;
		case PROFILE_EVT_SEND_DATA_COMPLETE:
			...
			bt_harmony_adapter_app_send_callback_msg(BHA_CALLBACK_MSG_INDICATION_SENT, callback_data);
			break;
		...
		case SERVICE_CALLBACK_TYPE_WRITE_CHAR_VALUE: {
			...
			if (p_harmony_cb_data->msg_data.write.write_cb) {
				...
				bt_harmony_adapter_app_send_callback_msg(BHA_CALLBACK_MSG_WRITE_SENT, MSG_DATA);
			} else {
