...
void bt_ota_central_client_app_handle_conn_mtu_info_evt(uint8_t conn_id, uint16_t mtu_size)
{
	...
	data_uart_print("bt_ota_central_client_app_handle_conn_mtu_info_evt: conn_id %d, mtu_size %d", conn_id, mtu_size);
            ...
            case GAP_ADTYPE_MANUFACTURER_SPECIFIC:
                {
                    if (memcmp(buffer + 2, p_mac_addr, length - 3) == 0)
                    {
						...
						data_uart_print("GAP_ADTYPE_MANUFACTURER_SPECIFIC: %s.\r\n", TRACE_BDADDR(buffer + 2));
                        return true;
                    }
