...
#if APP_GENERAL_CLIENT_TEST
T_CLIENT_ID     gcs_client_id = 0xf0;
#endif
#endif
...
#endif
    if (p_gap_data->msg_type == SERVICE_CALLBACK_TYPE_INDIFICATION_NOTIFICATION)
    {
        if (p_gap_data->msg_data.opcode == GATT_SERVICE_CHANGE_CCCD_ENABLE)
        {
            APP_PRINT_INFO0("GATT_SERVICE_CHANGE_CCCD_ENABLE");
        }
        else if (p_gap_data->msg_data.opcode == GATT_SERVICE_CHANGE_CCCD_DISABLE)
        {
            APP_PRINT_INFO0("GATT_SERVICE_CHANGE_CCCD_DISABLE");
        }
    }
    return result;
}
...
void gcs_handle_discovery_result(uint8_t conn_id, T_GCS_DISCOVERY_RESULT discov_result)
{
    ...
    case GCS_ALL_PRIMARY_SRV_DISCOV:
        ...
        for (i = 0; i < discov_result.result_num; i++)
        {
            ...
            case DISC_RESULT_ALL_SRV_UUID16:
                data_uart_print("ALL SRV UUID16[%d]: service range: 0x%4x-0x%4x, uuid16 0x%4x\r\n",
            ...
            case DISC_RESULT_ALL_SRV_UUID128:
                memcpy(uuid_128, &p_result_table->result_data.srv_uuid128_disc_data.uuid128[0], sizeof(uuid_128));
                data_uart_print("ALL SRV UUID128[%d]: service range: 0x%4x-0x%4x, service=<0x%8x%8x%8x%8x>\r\n",
    ...
    case GCS_BY_UUID128_SRV_DISCOV:
        ...
        for (i = 0; i < discov_result.result_num; i++)
        {
            ...
            case DISC_RESULT_SRV_DATA:
                data_uart_print("SRV DATA[%d]: service range: 0x%4x-0x%4x\r\n",
    ...
    case GCS_ALL_CHAR_DISCOV:
        ...
        for (i = 0; i < discov_result.result_num; i++)
        {
            ...
            case DISC_RESULT_CHAR_UUID16:
                ...
                data_uart_print("CHAR UUID16[%d]: decl_handle 0x%4x, properties 0x%4x, value_handle 0x%4x, uuid16 0x%4x\r\n",
            ...
            case DISC_RESULT_CHAR_UUID128:
                ...
                memcpy(uuid_128, &p_result_table->result_data.char_uuid128_disc_data.uuid128[0], sizeof(uuid_128));
                data_uart_print("CHAR UUID128[%d]:  decl hndl=0x%4x, prop=0x%4x, value hndl=0x%4x, uuid128=<0x%8x%8x%8x%8x>\r\n",
    ...
    case GCS_BY_UUID_CHAR_DISCOV:
        ...
        for (i = 0; i < discov_result.result_num; i++)
        {
            ...
            case DISC_RESULT_BY_UUID16_CHAR:
                properties = p_result_table->result_data.char_uuid16_disc_data.properties;
                data_uart_print("UUID16 CHAR[%d]: Characteristics by uuid16, decl hndl=0x%4x, prop=0x%4x, value hndl=0x%4x, uuid16=<0x%4x>\r\n",
                                i, p_result_table->result_data.char_uuid16_disc_data.decl_handle,
                                p_result_table->result_data.char_uuid16_disc_data.properties,
                                p_result_table->result_data.char_uuid16_disc_data.value_handle,
                                p_result_table->result_data.char_uuid16_disc_data.uuid16);
    ...
    case GCS_BY_UUID128_CHAR_DISCOV:
        ...
        for (i = 0; i < discov_result.result_num; i++)
        {
            ...
            case DISC_RESULT_BY_UUID128_CHAR:
                properties = p_result_table->result_data.char_uuid128_disc_data.properties;
                memcpy(uuid_128, &p_result_table->result_data.char_uuid128_disc_data.uuid128[0], sizeof(uuid_128));
                data_uart_print("UUID128 CHAR[%d]: Characteristics by uuid128, decl hndl=0x%4x, prop=0x%4x, value hndl=0x%4x, uuid128=<0x%8x%8x%8x%8x>\r\n",
                                i, p_result_table->result_data.char_uuid128_disc_data.decl_handle,
                                p_result_table->result_data.char_uuid128_disc_data.properties,
                                p_result_table->result_data.char_uuid128_disc_data.value_handle,
                                uuid_128[3], uuid_128[2], uuid_128[1], uuid_128[0]);
    ...
    case GCS_ALL_CHAR_DESC_DISCOV:
        ...
        for (i = 0; i < discov_result.result_num; i++)
        {
            ...
            case DISC_RESULT_CHAR_DESC_UUID16:
                data_uart_print("DESC UUID16[%d]: Descriptors handle=0x%4x, uuid16=<0x%4x>\r\n",
            ...
            case DISC_RESULT_CHAR_DESC_UUID128:
                memcpy(uuid_128, &p_result_table->result_data.char_desc_uuid128_disc_data.uuid128[0],
                       sizeof(uuid_128));
                data_uart_print("DESC UUID128[%d]: Descriptors handle=0x%4x, uuid128=<0x%8x%8x%8x%8x>\r\n",
    ...
    default:
        data_uart_print("Invalid disc type: conn_id %d, state %d\r\n",
...
T_APP_RESULT gcs_client_callback(T_CLIENT_ID client_id, uint8_t conn_id, void *p_data)
{
    ...
    if (client_id == gcs_client_id)
    {
        ...
        case GCS_CLIENT_CB_TYPE_READ_RESULT:
            data_uart_print("READ RESULT: cause 0x%4x, handle 0x%4x,  value_len %d\r\n",
        ...
        case GCS_CLIENT_CB_TYPE_WRITE_RESULT:
            data_uart_print("WRITE RESULT: cause 0x%4x ,handle 0x%4x, type %d\r\n",
        ...
        case GCS_CLIENT_CB_TYPE_NOTIF_IND:
            ...
            {
                data_uart_print("INDICATION: handle 0x%4x, value_size %d\r\n",
            ...
            {
                data_uart_print("NOTIFICATION: handle 0x%4x, value_size %d\r\n",
                                p_gcs_cb_data->cb_content.notif_ind.handle,
                                p_gcs_cb_data->cb_content.notif_ind.value_size);
